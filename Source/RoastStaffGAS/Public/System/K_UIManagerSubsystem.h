 ...
 */
UCLASS()
class ROASTSTAFFGAS_API UK_UIManagerSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
    ...
    int32 CalculateZOrder(UK_BaseWidget* Widget) const;
     ...
     * - 중복 로직 방지 및 유지보수성 향상
     */
    void CloseUIInternal(UK_BaseWidget* Widget);
...
public:
     ...
     */
    void CloseUI(UK_BaseWidget* Widget);
...
private:
    ...
    UPROPERTY()
    TMap<TSubclassOf<UK_BaseWidget>, UK_BaseWidget*> PersistentUIMap;
     ...
     */
    UPROPERTY()
    TArray<UK_BaseWidget*> PopupUIStack;
     ...
     */
    UPROPERTY()
    TMap<TSubclassOf<UK_BaseWidget>, UK_BaseWidget*> CachedWidgets;
};

// 템플릿 함수 구현
template<typename T>
T* UK_UIManagerSubsystem::GetOrCreateWidget(TSubclassOf<T> WidgetClassFactory)
{
    TSubclassOf<UK_BaseWidget> TargetClassFactory = WidgetClassFactory;
...
template<typename T>
T* UK_UIManagerSubsystem::OpenUI(TSubclassOf<T> TargetClassFactory)
{
    ...
    UK_BaseWidget* BaseWidget = Widget;
    
    // Persistent 타입 UI일 때
    if (BaseWidget->UILayer == EUILayer::PERSISTENT)
    {
        TSubclassOf<UK_BaseWidget> BaseClassFactory = TargetClassFactory;
...
template<typename T>
void UK_UIManagerSubsystem::CloseUI(TSubclassOf<T> targetClassFactory)
{
	if (!targetClassFactory)
	{
		return;
	...
	UK_BaseWidget* widget = CachedWidgets[baseClassFactory];
